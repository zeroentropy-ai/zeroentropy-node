...
import { APIUserAbortError } from 'zeroentropy';
import { Headers } from 'zeroentropy/core';
...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['ZEROENTROPY_BASE_URL'] = undefined;
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new ZeroEntropy({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['ZEROENTROPY_BASE_URL'] = 'http://localhost:5000/env';
      const client = new ZeroEntropy({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
